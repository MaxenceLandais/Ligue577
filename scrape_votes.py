...
def load_existing_votes(filepath="votes.json"):
    try:
...
def scrape_depute_votes(depute_id, datan_url):
    ...
    with sync_playwright() as p:
        ...
        print(f"🌐 Navigation vers : {votes_url}")
        page.goto(votes_url, wait_until="domcontentloaded")

        try:
            page.wait_for_selector("text=votes", timeout=10000)
        except Exception:
            pass
        ...
        # Extraction JS sans erreur de syntaxe
        js_script = r'''() => {
            const results = [];
        ...
        votes_list = page.evaluate(js_script)
        browser.close()

    all_votes = load_existing_votes("votes.json")
    ...
    print(f"✅ {len(votes_list)} votes enregistrés pour « {depute_id} » dans votes.json")


if __name__ == "__main__":
    depute_id = "gerault-verny"
    datan_url = "https://datan.fr/deputes/bouches-du-rhone-13/depute_gerault-verny"

    scrape_depute_votes(depute_id, datan_url)